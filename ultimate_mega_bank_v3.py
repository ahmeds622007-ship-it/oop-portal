...
public:
    ...
    StringBox(const StringBox& other) {
        text = new char[strlen(other.text) + 1]; // Allocate NEW memory
        strcpy(text, other.text); // Copy actual data, not just the pointer address
...
int main() {
    StringBox box1("Hello OOP");
    // This calls the copy constructor. If it was shallow, both would point to same memory!
    StringBox box2 = box1; 
    
    box1.print();
    box2.print();
    // When main ends, both destructors safely delete their own distinct memory.
    return 0;
}""",
...
public:
    Engine(int h) { hp = h; }
    void showHp() { cout << "Engine HP: " << hp << endl; }
};
...
public:
    Wheels(int s) { size = s; }
    void showSize() { cout << "Wheel Size: " << size << " inches" << endl; }
};
...
public:
    ...
    Car(int h, int s, string b) : Engine(h), Wheels(s) {
        brand = b;
    }
    ...
    void displayInfo() {
        ...
        showHp();   // From Engine
        showSize(); // From Wheels
    }
};

int main() {
    Car myCar(500, 19, "Ferrari");
    myCar.displayInfo();
    return 0;
}""",
...
private:
    int loadCapacity;
public:
    Truck(int s, int l) : Vehicle(s) { loadCapacity = l; }
    void show() { cout << "Truck Speed: " << speed << ", Load: " << loadCapacity << " Tons" << endl; }
};
...
int main() {
    ...
    // Polymorphism test
    MathBase* ptr = &obj;
    ptr->compute(10); // Late binding -> calls Derived
...
print("Ultimate Mega Bank V3 Generated. Filled ALL gaps requested by user's syllabus.")
